...
const Activities = ({ isMobile }: ActivitiesProps) => {
    const activities = [
        ...
        {
            "title": "Игры ГТО",
            "desc": `Все мы знаем, что такое комплекс ГТО, но что же такое Игры ГТО? Рассказываем! Игры ГТО — это новый зрелищный вид спорта на базе нормативов ГТО. На отдельных дорожках для каждого участника установлены спортивные снаряды, которые необходимо пройти всей команде. Тут и упражнения на скорость, силу, гибкость и даже киберспорт.
            ...
            >
                {activities.map((activity, index) => (
                    <SwiperSlide key={index} style={{ width: isMobile ? "400px" : "1100px" }}>
                        <Card isMobile={isMobile} title={activity.title} url={activity.url} priduUrl={activity.priduUrl} desc={activity.desc} img={activity.img} date={activity.date} />
                    </SwiperSlide>
                ))}
            </Swiper>
