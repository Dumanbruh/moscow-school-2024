...
const FreeStyle = ({ isMobile }: FreeStyleProps) => {
    const activities = [
        ...
        {
            "title": "Беговелы",
            "desc": "Приглашаем самых маленьких любителей активного отдыха принять участие в захватывающих соревнованиях на беговелах! Будет весело, энергично и незабываемо. Ждем всех на старте!",
                    ...
                    >
                        {activities.map((activity, index) => (
                            <SwiperSlide key={index} style={{ width: isMobile ? "400px" : "1100px" }}>
                                <Card isMobile={isMobile} title={activity.title} priduUrl={activity.priduUrl} desc={activity.desc} img={isMobile && activity.imgMobile ? activity.imgMobile : activity.img} date={activity.date} />
                            </SwiperSlide>
                        ))}
                    </Swiper>
